<?php
    ...
    $r = $obj->addOrder($customerid);
      ...
      echo "<script type='text/javascript'>window.location.href='task3-orders.php';</script>";
		}else{
	...
	function addPart(){
		...
		if ($obj->connect()){
			$queryStr = "INSERT INTO `coredb`.`odetails` (`ono`, `pno`, `qty`) VALUES ('{$OrderNo}', '{$PartNo}', '{$OrderQty}');";
			...
			if($result){
				...
				echo "<script type='text/javascript'>window.location.href='orders-items.php?oid={$OrderNo}';</script>";
			} else {
